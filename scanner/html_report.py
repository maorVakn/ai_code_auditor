...
<head>
  ...
  <style>
    :root {{ color-scheme: light; --bg:#f6f7f9; --panel:#fff; --text:#17202a; --muted:#5d6978; --line:#d9dee7; --high:#b42318; --med:#b54708; --low:#175cd3; --ok:#067647; }}
    * {{ box-sizing: border-box; }}
    ...
    .toolbar, .summary, .finding {{ background:var(--panel); border:1px solid var(--line); border-radius:8px; }}
...
<body>
  <header>
    ...
    <p class="meta">Path: {html.escape(str(summary.get("scanned_path", "")))} · AI enriched: {html.escape(str(report.get("ai_enriched", False)))}</p>
  </header>
  <main>
...
def _finding_card(finding: dict) -> str:
    ...
    prompt = html.escape(str(finding.get("safe_for_ai_copy", "")), quote=True)
    refs = " ".join(f"<a href=\"{html.escape(url, quote=True)}\">reference</a>" for url in finding.get("references", []) if url)
    return f"""<article class="finding" data-index="{index}">
  <div class="finding-head"><div><h2>{title}</h2><div class="loc">{loc} · {html.escape(str(finding.get("tool")))} · {html.escape(str(finding.get("rule_id")))}</div></div><span class="badge {severity}">{severity}</span></div>
