...
re-run bandit/detect-secrets/semgrep on the other ninety-nine, or re-hit
...
def get_cached_file_findings(cache: dict, file_path: str, current_hash: str) -> dict | None:
    """
    Returns the cached {"bandit": [...], "secrets": [...], "semgrep": [...]}
    for a file only if its hash matches the cached one (i.e. content
    unchanged). Returns None if there's no valid cache entry (new or
    changed file).
    """
    entry = cache["files"].get(file_path)
    if entry is not None and entry.get("hash") == current_hash:
        return {
            "bandit": entry.get("bandit", []),
            "secrets": entry.get("secrets", []),
            "semgrep": entry.get("semgrep", []),
        }
    return None
